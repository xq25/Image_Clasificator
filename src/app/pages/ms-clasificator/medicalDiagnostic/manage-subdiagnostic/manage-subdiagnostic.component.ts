...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, MatButtonModule, MatIconModule, MatCardModule, MatProgressSpinnerModule, MatListModule, MatInputModule, MatFormFieldModule],
...
export class ManageSubDiagnosticComponent implements OnInit {
  ...
  loadSubs(): void {
    ...
    // Assuming findById returns sub-diagnostics in response.data.subDiagnostics
    this.medicalDiagnosticService.findById(Number(this.parentId)).subscribe({
      next: (res: any) => {
        const subs: MedicalDiagnostic[] = res?.data?.subDiagnostics ?? [];
